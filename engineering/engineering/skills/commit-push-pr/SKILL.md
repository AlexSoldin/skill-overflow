...
After the PR is created, check if the commit includes Django migration files:

1. Run `git diff HEAD~1 --name-only` and look for files matching `*/migrations/*.py`
2. If migration files are found:
   - Extract the Linear ticket ID from the branch name (pattern: `CS-\d+`)
   - If a ticket ID is found, add the `has-migration` label to the Linear ticket via `mcp__plugin_linear_linear__save_issue`
   - First check if the `has-migration` label exists with `mcp__plugin_linear_linear__list_issue_labels`, create it with `mcp__plugin_linear_linear__create_issue_label` if needed
3. This is a silent step — no user interaction unless the label operation fails
